...
export default function App() {
  ...
  const checkLoginStatus = async () => {
    ...
    try {
      ...
      if (email) {
        const response = await axios.post("http://192.168.29.253/checkV", {
          email,
        });
  ...
  const handleForgotPassword = async (email) => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.29.253/forgot-password",
        { email }
      );
      if (response.status === 200) {
        Alert.alert(
      ...
      Alert.alert(
        "Error",
        `Unable to send password reset email ${error.response.data.error}`
      );
    } finally {
      setIsLoading(false); // Ensure loading state is reset
    }
  ...
  return (
    <SafeAreaView
      style={{ flex: 1, paddingTop: 10, backgroundColor: "#ffffff" }}
    >
      <View style={{ flex: 1 }}>

...
const Newtrip = ({ navigation }) => {
  ...
  const handlePlaceSelect = async (place, dayIndex, placeIndex) => {
    ...
    const newDays = [...days];
    newDays[dayIndex].places[placeIndex].selected = {
      ...placeDetails,
          ...
          <View className="px-5">
            ...
            <View className="flex flex-row flex-wrap mt-3">
              <View className="basis-1/2 pr-2">
                <Text style={[styles.popsemi, { fontSize: 16 }]}>
                  No of days :
                </Text>
                <TextInput
                  style={[styles.popreg, { fontSize: 18 }]}
                ...
                {day.places.map((place, placeIndex) => (
                  <View key={placeIndex}>
                    ...
                    <TextInput
                      ...
                      value={place.query}
                      onChangeText={(text) =>
                        handleInputChange(text, dayIndex, placeIndex)
                      }
                      placeholder={`Search for place ${placeIndex + 1}`}
                    />
                    {activeInput.day === dayIndex &&
                      activeInput.placeIndex === placeIndex && (
                        <FlatList
                          ...
                          renderItem={({ item }) => (
                            <Pressable
                              onPress={() =>
                                handlePlaceSelect(item, dayIndex, placeIndex)
                              }
                            >
